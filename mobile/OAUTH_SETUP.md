...
EXPO_PUBLIC_KAKAO_CLIENT_ID=...        # REST API 키 (웹/iOS)
EXPO_PUBLIC_KAKAO_CLIENT_SECRET=...    # 카카오 콘솔에서 "Client Secret" 사용 설정한 경우만
EXPO_PUBLIC_KAKAO_NATIVE_APP_KEY=...   # Android 네이티브 전용, REST API 키와 다른 값 — 아래 Kakao 섹션 참고

EXPO_PUBLIC_NAVER_CLIENT_ID=...      # 웹은 Client Secret 불필요, Android 네이티브는 아래 SECRET도 필요
EXPO_PUBLIC_NAVER_CLIENT_SECRET=...  # Android 네이티브 전용 — 앱 바이너리에 임베딩됨, 아래 Naver 섹션의 트레이드오프 설명 참고
```
...
- **네이티브(iOS/Android)**: Expo Go에서는 커스텀 스킴(`workproof://`)이 동작하지 않습니다. `npx expo run:ios` / `npx expo run:android`로 빌드하거나 EAS 개발 빌드를 사용해야 실제 테스트가 가능합니다. 이 경우 리다이렉트 URI는 `workproof://`입니다(`AuthSession.makeRedirectUri({ scheme: 'workproof' })`가 dev-client/standalone 빌드에서 반환하는 값 — Expo Go에서는 대신 `exp://...` 형태가 나옵니다).
  - 이 리다이렉트 URI는 **Google(모든 플랫폼)과 Kakao/Naver의 웹·iOS 플로우**에만 해당합니다. **Kakao/Naver의 Android 플로우는 브라우저 리다이렉트를 아예 쓰지 않고 네이티브 SDK**(`src/auth/kakaoNative.ts`, `src/auth/naverNative.ts`)로 동작하므로, `workproof://` 리다이렉트 URI 등록이 필요 없는 대신 각 콘솔에 패키지명 + 키 해시(Kakao) / Android 플랫폼(Naver) 등록이 필요합니다 — 아래 3절 참고.
...
6. **웹/iOS 개발 빌드**: 위 REST API 키(`EXPO_PUBLIC_KAKAO_CLIENT_ID`)로 브라우저 기반 AuthSession/PKCE
   플로우를 그대로 씁니다(`src/auth/socialLogin.ts`). "카카오 로그인" → Redirect URI 목록에
   **`workproof://`** 를 웹 주소와 별개 항목으로 추가 등록하세요.
7. **Android 개발 빌드(네이티브 SDK)**: Android는 브라우저 리다이렉트 대신
   ...
   `npx expo run:android`로 만든 네이티브 빌드가 반드시 필요합니다.
   ...
   - "앱 키" 중 **네이티브 앱 키**(REST API 키와는 다른 값)를 `EXPO_PUBLIC_KAKAO_NATIVE_APP_KEY`에 입력.
     이 값은 `app.config.ts`의 `@react-native-seoul/kakao-login` 플러그인 설정을 통해 빌드타임에
     AndroidManifest/strings.xml에 심어집니다 — **값을 바꾸면 네이티브 프로젝트를 다시 생성해야 하므로
     EAS 재빌드가 필요합니다.**
   - Kakao SDK 아티팩트는 Maven Central에 없어서 `expo-build-properties`로 카카오 전용 Maven
     ...
     (`app.config.ts` 참고, 별도 설정 불필요).

### Naver

**웹**은 JS SDK로, **Android**는 공식 네이티브 SDK([`@react-native-seoul/naver-login`](https://github.com/crossplatformkorea/react-native-naver-login))로 동작합니다. **iOS는 아직 미구현**입니다(`src/auth/socialLogin.ts`의 `loginWithNaver()`가 iOS에서 구체적 사유와 함께 `not_configured`를 반환).
...
**Android 네이티브 SDK 연동과 보안 트레이드오프 (중요):**

이 프로젝트는 원래 "Secret을 앱/EXPO_PUBLIC 환경변수에 넣지 않는다"는 원칙 때문에 Android 네이버
로그인을 보류해왔습니다. 네이버 오픈API의 정식 서버 인증 플로우는 토큰 교환 시 `client_id` +
**`client_secret`** + `code`를 함께 보내야 하고, 네이버 공식 [Android SDK](https://github.com/naver/naveridlogin-sdk-android)도
`NaverIdLoginSDK.initialize(context, clientId, clientSecret, clientName)`처럼 **Client Secret을
...
감수하기로 결정했습니다** — 백엔드 없이 지금 바로 Android 네이버 로그인을 동작시키기 위해서입니다.
...
- `EXPO_PUBLIC_NAVER_CLIENT_SECRET`은 EAS 빌드 시 `@react-native-seoul/naver-login`의
  ...
  포함**됩니다. APK를 디컴파일하면 이 값을 추출할 수 있습니다 — Google/Kakao의 REST API 키(공개돼도
  안전하도록 설계된 값)와는 성격이 다른, 진짜 시크릿이 노출되는 것입니다.
...
1. [네이버 개발자센터](https://developers.naver.com/apps) → 해당 앱 → "API 설정"에서 **Android** 플랫폼
   ...
   가능한 경우가 많음 — 콘솔 안내 참고)
2. "네이버 로그인" 설정에서 발급된 **Client Secret**을 `EXPO_PUBLIC_NAVER_CLIENT_SECRET`에 입력
3. Expo config plugin(`app.config.ts`의 `@react-native-seoul/naver-login`)이 `urlScheme: "workproof"`로
   네이티브 프로젝트를 설정합니다 — 이 값을 바꾸면 EAS 재빌드가 필요합니다.
4. `src/auth/naverNative.ts`가 `NaverLogin.initialize()` → `NaverLogin.login()` → `NaverLogin.getProfile()`
   순서로 로그인과 프로필 조회를 모두 처리합니다(별도 REST 호출 불필요, SDK가 감싸줌).
